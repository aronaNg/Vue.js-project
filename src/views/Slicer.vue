<template>
  <div>
    <h1>Labo</h1>
    <v-simple-table :style="{ margin:'100px', backgroundColor:'#EF5350'}">
      <template>
      <tr>
        <td><h1>Viruses</h1></td>
      </tr>
    ...
    <label for="cut">part length: </label><input id="cut" v-model.number="cutFactor"><v-btn :style="{background:'#B71C1C'}"   :disabled="chosenViruses.length==0" @click="cut()">Cut</v-btn>
    <label for="mute">nb mutations: </label><input id="mute" v-model.number="nbMutation"><v-btn :style="{background:'#B71C1C'}"   :disabled="chosenViruses.length==0" @click="mutation()">Mutation</v-btn>
    <hr/>
    <v-btn color="blue" elevation="2" small @click="$router.push({path:'/labo/mix'})">Go to mixer</v-btn>
  </div>
</template>
